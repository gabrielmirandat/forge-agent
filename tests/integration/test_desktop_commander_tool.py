"""Integration test for desktop_commander MCP tools.

Tests verify that the LLM correctly decides to call desktop_commander tools
...
async def desktop_commander_tools(test_workspace: Path) -> List[Any]:
    """Get tools from desktop_commander MCP server."""
    docker_command = [
        "docker",
        "run",
        "-i",
    ...
    assert len(tools) > 0, "No tools loaded from desktop_commander"
    return tools
...
async def test_list_directory_tool_decision(
    desktop_commander_tools: List[Any],
    llm: ChatOllama,
    test_workspace: Path,
):
    ...
    list_dir_tool = None
    for tool in desktop_commander_tools:
        if "list_directory" in tool.name.lower() or "list_dir" in tool.name.lower():
            list_dir_tool = tool
            break
    
    assert list_dir_tool is not None, f"list_directory tool not found. Available: {[t.name for t in desktop_commander_tools[:10]]}"
    ...
    agent_executor = create_agent_for_tools(
        ...
        system_prompt=(
            ...
            "Always use /workspace as the base path."
        ),
    )
...
async def test_write_file_tool_decision(
    ...
    assert write_tool is not None, f"write_file tool not found. Available: {[t.name for t in desktop_commander_tools[:10]]}"
    ...
    agent_executor = create_agent_for_tools(
        ...
        system_prompt=(
            ...
            "Use /workspace as the base path for files."
        ),
    )
    ...
    result = await agent_executor.ainvoke(
        {"input": f"Create a file named {test_file.name} with content 'Hello from tool decision test'"},
    )
...
async def test_read_file_tool_decision(
    ...
    assert read_tool is not None, f"read_file tool not found. Available: {[t.name for t in desktop_commander_tools[:10]]}"
    ...
    # Execute
    result = await agent_executor.ainvoke(
        {"input": f"Read the file /workspace/{test_file.name}"},
    )
